...
(*Funkcja która dodaje przedział do drzewa, uwzględniając to, 
  że przedziały mogą na siebie nachodzić*)    
let rec add (minv,maxv) t =
  match t with
  | Empty -> Node (Empty, (minv, maxv), Empty, 1, maxv ++ (-minv) ++ 1)
  | Node (l, (lv, rv), r, h, _) -> 
    let nl, l_cmp, _ = split minv t
    and _, r_cmp, nr = split maxv t in
    let news = (minv, maxv) in
      match nl, nr with
      | Empty, Empty ->  
      ...
      | Empty, pp -> 
        ...
        if overlap_sum minpp news 
            then add_one (sum minpp news) (remove_min_elt pp)
        else add_one news pp
      | ll, Empty -> 
        let maxll = max_elt ll in
        if overlap_sum (maxll) news 
            then add_one (sum (maxll) news) (remove_max_elt ll)
